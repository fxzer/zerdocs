...
const isValid = function (str) {
  ...
  for (let i = 0; i < str.length; i++) {
    if (str[i] === "(" || str[i] === "{" || str[i] === "[") {
      ...
      if (map[str[i]] != arr.pop()) return false;
    }
  }
  // arr数组已全部匹配，出完栈
...
function findMedianSortedArrays(nums1, nums2) {
  ...
  if (len % 2 ==== 0) return (mergeArr[len / 2 - 1] + mergeArr[len / 2]) / 2;
...
class EventEmitter {
  ...
  on(eventName, listener) {
    if (!this.registry[eventName]) this.registry[eventName] = [];
...
eventEmitter.on("eventA", () => {
  console.log("eventA triggered");
});
eventEmitter.on("eventB", () => {
  console.log("eventB triggered");
});
...
const p3 = Promise.resolve("error");
...
function numberToChinese(num, upper = false) {
  if (!/^(\-|\+)?\d*(\.\d*)?$/.test(num)) return "不是一个合法的数字！";
...
function numberToChinese(num) {
  if (num ==== 0) return "零";
  // 数字超过999999999999则报错
  if (num.toString().length > 12) throw new Error("数字过大，无法转换");
  ...
  function transform(str) {
    ...
    for (let i = 0; i < str.length; i++) {
      ...
      if (digit ==== 0) {
        ...
        if (res[res.length - 1] !=== "零") res += char;
      } else {
        res += char + unit;
      }
    }
    if (res[res.length - 1] ==== "零") res = res.slice(0, res.length - 1);
